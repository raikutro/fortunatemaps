...
function makeMapChunk(tileMap, points) {
	...
	for (const point of points) {
		...
		if ([TILE_IDS.REDFLAG, TILE_IDS.BLUEFLAG, TILE_IDS.YELLOWFLAG].includes(tileType)) {
			mapChunk.type = 'base';
...
async function extractMapChunks(tileMap) {
	const keyElementMask = TPAnalysis.createCustomTileMask(tileMap, {
		...
		[TPAnalysis.TILE_IDS.REDENDZONE]: 2,
		[TPAnalysis.TILE_IDS.BLUEENDZONE]: 2,

		[TPAnalysis.TILE_IDS.YELLOWFLAG]: 3,
	...
	if (goals.length >= 2) {
		...
		// If there are fewer than 2 goals (e.g. single center flag), keep all chunks
		for (const [label, points] of labeledPoints) {
			try {
				mapChunks.push(makeMapChunk(tileMap, points));
			} catch (e) {
				console.log('Failed to make a chunk: ', e);
			}
		}
